...
func NewLifecycleContext() LifecycleContext {
	utils.DebugLog("Creating new lifecycle context")
...
func (ctx *lifecycleContextImpl) Shutdown() []error {
	utils.DebugLog("[Context ID: %s] Closing lifecycle context...", ctx.ID())
	...
	for _, k := range cacheKeys {
		utils.DebugLog("[Context ID: %s] Deleting instance for service type: %v", ctx.ID(), k)
		...
		go func(lm LifecycleListener, k reflect.Type, ctx *lifecycleContextImpl) {
			...
			defer func() {
				if r := recover(); r != nil {
					utils.DebugLog("[Context ID: %s] Recovered from panic in EndLifecycle for service type: %v, panic: %v", ctx.ID(), k, r)
			...
			utils.DebugLog("[Context ID: %s] Ending lifecycle for service type: %v...", ctx.ID(), k)
			if err := lm.EndLifecycle(); err != nil {
				utils.DebugLog("[Context ID: %s] Error ending lifecycle for service type: %v, error: %v", ctx.ID(), k, err)
			...
			utils.DebugLog("[Context ID: %s] Removing instance for service type: %v", ctx.ID(), k)
			ctx.mutex.Lock()
			delete(ctx.cache, k)
			ctx.mutex.Unlock()
		}(lm, k, ctx)
	...
	utils.DebugLog("[Context ID: %s] Lifecycle context closed", ctx.ID())
	return errors
}
...
func (ctx *lifecycleContextImpl) GetInstance(serviceType reflect.Type) (reflect.Value, bool) {
	...
	utils.DebugLog("[Context ID: %s] Getting instance for service type: %v", ctx.ID(), serviceType)
	instance, exists := ctx.cache[serviceType]
	if exists {
		utils.DebugLog("[Context ID: %s] Instance found for service type: %v", ctx.ID(), serviceType)
	} else {
		utils.DebugLog("[Context ID: %s] No instance found for service type: %v", ctx.ID(), serviceType)
	}
...
func (ctx *lifecycleContextImpl) SetInstance(serviceType reflect.Type, instance reflect.Value) {
	...
	utils.DebugLog("[Context ID: %s] Setting instance for service type: %v", ctx.ID(), serviceType)
	if _, exists := ctx.cache[serviceType]; exists {
		utils.DebugLog("[Context ID: %s] Overwriting existing instance for service type: %v", ctx.ID(), serviceType)
	}
	ctx.cache[serviceType] = instance
	utils.DebugLog("[Context ID: %s] Instance set for service type: %v", ctx.ID(), serviceType)
}
